function drawSketch(canvasId, item) {
    ...
    if (item.category === 'glass') {
        ...
        if (item.w > 0 && item.h > 0) {
            ...
            ctx.fillText('Без размеров', x + w / 2, y + h - 6);
        }
    } else if (item.category === 'hardware') {
        ctx.font = 'bold 72px sans-serif'; ctx.fillStyle = '#94a3b8'; ctx.textAlign = 'center';
        ctx.fillText('🔧', x + w / 2, y + h / 2 + 25);
    } else if (item.category === 'glasses') {
        ctx.fillStyle = '#e0f2fe'; ctx.fillRect(x, y, w, h); ctx.strokeRect(x, y, w, h);
        ctx.lineWidth = 3; ctx.strokeStyle = '#0284c7'; ctx.strokeRect(x + 4, y + 4, w - 8, h - 8);
        ctx.beginPath(); ctx.strokeStyle = 'rgba(255,255,255,0.8)'; ctx.lineWidth = w / 3;
        ctx.moveTo(x + w * 0.1, y + h - 10); ctx.lineTo(x + w - 10, y + h * 0.1); ctx.stroke();
        ctx.font = 'bold 12px sans-serif'; ctx.fillStyle = '#0f172a'; ctx.textAlign = 'center';
        ctx.fillText(item.thickness + 'мм', x + w / 2, y + h / 2 + 5);
        ...
        // Рисуем диагональное разделение для показа двух цветов
        ctx.fillStyle = cOut; ctx.beginPath(); ctx.moveTo(x, y); ctx.lineTo(x + w, y); ctx.lineTo(x, y + h); ctx.fill();
        ctx.fillStyle = cIn; ctx.beginPath(); ctx.moveTo(x + w, y); ctx.lineTo(x + w, y + h); ctx.lineTo(x, y + h); ctx.fill();
        
        ctx.lineWidth = 4; ctx.strokeStyle = '#1e293b'; ctx.strokeRect(x, y, w, h);
        ctx.beginPath(); ctx.lineWidth = 1; ctx.strokeStyle = '#94a3b8'; ctx.moveTo(x, y + h); ctx.lineTo(x + w, y); ctx.stroke();
